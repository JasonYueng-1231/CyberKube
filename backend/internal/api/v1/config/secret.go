...
func RegisterSecret(r *gin.RouterGroup) {
    g := r.Group("/secrets")
    g.GET("", func(c *gin.Context) {
        cluster := c.Query("cluster"); ns := c.Query("namespace"); if ns=="" { ns = "default" }
        items, err := service.ListSecrets(cluster, ns)
    ...
    g.GET("/detail", func(c *gin.Context) {
        ...
        d, err := service.GetSecret(cluster, ns, name)
        if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"code":50003, "message": err.Error()}); return }
    ...
    g.POST("/yaml", func(c *gin.Context) {
        ...
        if err := service.CreateSecretYAML(req.Cluster, req.Namespace, req.Yaml); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code":40901, "message": err.Error()}); return }
    ...
    g.PUT("/yaml", func(c *gin.Context) {
        ...
        if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code":40001, "message":"参数错误"}); return }
        if req.Namespace=="" { req.Namespace = "default" }
        if err := service.UpdateSecretYAML(req.Cluster, req.Namespace, req.Yaml); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code":40901, "message": err.Error()}); return }
        c.JSON(http.StatusOK, gin.H{"code":0, "message":"success"})
    })
    g.DELETE("", func(c *gin.Context) {
        cluster := c.Query("cluster"); ns := c.Query("namespace"); name := c.Query("name"); if ns=="" { ns = "default" }
        if err := service.DeleteSecret(cluster, ns, name); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"code":50003, "message": err.Error()}); return }
        c.JSON(http.StatusOK, gin.H{"code":0, "message":"success"})
    })
}
