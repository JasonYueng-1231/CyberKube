...
export default function ConfigMapList() {
  ...
  const load = async () => { if (!cluster) return; setLoading(true); try { const d = await api(`/configmaps?cluster=${cluster}&namespace=${namespace}`); setItems(d.items||[]); } catch(e:any){ message.error(e.message) } finally { setLoading(false); } };
  useEffect(()=>{ load(); }, [cluster, namespace]);
  const del = async (name:string) => { Modal.confirm({ title:`删除 ${name}?`, onOk: async ()=>{ await api(`/configmaps?cluster=${cluster}&namespace=${namespace}&name=${name}`, { method:'DELETE' }); message.success('已删除'); load(); } }); };

  return (
    <Card className="cyber-card" title="ConfigMap 列表" extra={<Button type="primary" onClick={()=> { setEditName(undefined); setOpen(true); }}>新建 ConfigMap</Button>}>
      ...
      />
      <GenericYamlModal open={open} onClose={()=> { setOpen(false); load(); }} cluster={cluster} namespace={namespace} name={editName} kind='configmap' />
    </Card>
  );
}
