...
export default function IngressList() {
  ...
  const load = async () => {
    ...
    try { const d = await api(`/ingresses?cluster=${cluster}&namespace=${namespace}`); setItems(d.items||[]); }
    catch (e:any) { message.error(e.message); }
    finally { setLoading(false); }
  };
  useEffect(() => { load(); }, [cluster, namespace]);

  const del = async (name:string) => { Modal.confirm({ title:`删除 ${name}?`, onOk: async ()=>{ await api(`/ingresses?cluster=${cluster}&namespace=${namespace}&name=${name}`, { method:'DELETE' }); message.success('已删除'); load(); } }); };

  return (
    <Card className="cyber-card" title="Ingress 列表" extra={<Button type="primary" onClick={()=> { setEditName(undefined); setOpen(true); }}>新建 Ingress</Button>}>
      ...
      <Table rowKey={(r:any)=> r.metadata?.namespace+"/"+r.metadata?.name} loading={loading} dataSource={items} pagination={false}
        columns={[
          ...
          { title:'主机', render: (_:any,r:any)=> (r.spec?.rules||[]).map((h:any)=> h.host || '-').join(', ') },
          { title:'服务', render: (_:any,r:any)=> {
            const paths = (r.spec?.rules||[]).flatMap((ru:any)=> ru.http?.paths||[]);
            if (!paths.length) return '-';
            return paths.map((p:any, idx:number)=> <Tag key={idx}>{p.backend?.service?.name}:{p.backend?.service?.port?.number || p.backend?.service?.port?.name}</Tag>);
          }},
      ...
      />
      <GenericYamlModal open={open} onClose={()=> { setOpen(false); load(); }} cluster={cluster} namespace={namespace} name={editName} kind='ingress' />
    </Card>
  );
}
